...
14. 部门与业务域使用独立配置界面维护，一个部门可绑定多个业务域；热度半衰期、首页推荐来源权重、稳定打散分差阈值和变化周期均可配置。
...
| MySQL/DM8 | 部门业务域绑定、推荐文件投影、现有文件/空间业务域数据 |
...
新增表 `department_business_domain`：
...
| `business_domain_code` | varchar(16) | 业务域编码，统一大写 |
| `create_user` | int nullable | 配置人 |
...
门户新增独立的“部门业务域绑定”配置界面，不把部门绑定塞进现有业务域卡片/空间绑定表单。页面规则：

- 按部门展示一行绑定关系，部门只能精确选择一个。
- 业务域使用多选控件，可为一个部门绑定一个或多个现有业务域。
- 不提供“包含子部门”开关；父子部门分别维护。
- 保存前校验部门不重复、业务域编码存在且去重；允许清空某部门绑定。
...
门户保存部门业务域绑定后，以全量替换方式同步到 BiSheng，避免增量漏删。现有 `DomainConfig` 继续负责业务域展示信息和知识空间绑定，两类配置职责分离。
...
部门绑定与推荐配置都放在完整聚合配置的 `portal` 节点中，例如：
...
语义：当前租户的绑定全量替换，未出现在请求中的旧绑定被删除；每条绑定只作用于指定部门，一个部门可绑定多个业务域，不继承父部门或子部门。BiSheng 在同一数据库事务中保存聚合配置与绑定明细，版本由服务端单调递增并随响应返回；门户不得拆成两个保存请求。候选池任务和在线推荐使用同一配置版本。
...
| 层 | 改动 |
|---|---|
| Schema | `personalized_v1` 推荐模式、部门多业务域绑定 DTO、推荐算法配置 DTO、`portal_search` 事件 |
| Model/Repository | `department_business_domain`、租户推荐算法配置、`portal_recommendation_file_projection` 及 Alembic；复用现有文件业务域解析逻辑 |
...
- `src/backend/bisheng/database/models/department.py`：仅复用部门实体，部门与业务域绑定建独立表，不往部门表塞 JSON。
- `src/backend/bisheng/core/database/alembic/versions/`：增加部门绑定和推荐投影迁移；复用已有 `v2_6_0_f050_knowledge_business_domain_codes.py`。

建议新增代码落点：

- `src/backend/bisheng/knowledge/domain/services/portal_recommendation_service.py`：候选召回、打分、已读策略、权限后置过滤和降级。
- `src/backend/bisheng/knowledge/domain/models/department_business_domain.py` 和 `portal_recommendation_file_projection.py`：领域模型。
- `src/backend/bisheng/knowledge/domain/repositories/interfaces/`：部门绑定、投影和推荐查询接口。
...
| 管理后台 | 新增独立“部门业务域绑定”界面，部门单选、业务域多选且不提供继承开关；推荐配置增加数量及四个算法参数 |
| 配置同步 | 复用现有聚合配置 PUT，一次同步完整 portal 配置、部门业务域绑定和推荐算法配置到 BiSheng |
...
- `backend/app/schemas/portal_config.py`、`backend/app/config/portal_config.py` 与 `frontend/src/pages/AdminPage.tsx`：增加独立的部门业务域绑定数据和配置界面；在现有 `RecommendationConfig` 增加 `home_total_count=20`、`hot_half_life_days=7`、`home_entry_source_weight=0.3`、`stable_shuffle_score_gap=5`、`stable_shuffle_cycle_days=7` 及对应范围校验。
...
| AC-24 | 管理员维护部门业务域绑定 | 可为一个部门多选业务域，不提供子部门继承；保存后全量同步且旧绑定正确删除 |
...
2. 部门业务域不继承子部门；新增独立的部门业务域绑定配置界面，并支持一个部门绑定多个业务域。
