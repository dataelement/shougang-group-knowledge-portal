...
def test_merge_rest_auth_maps_to_oauth_fields():
    ...
    assert "redirect_uri" not in unified or unified.get("redirect_uri") in ("", None)
...
def test_merge_rest_auth_preserves_existing_redirect_uri():
    ...
    assert unified["redirect_uri"] == "http://portal.example.com/api/v1/auth/unified/callback"
    assert unified["provider"] == "custom"
    assert '"rest_base_url":"https://iam.example.com"' in unified["state_secret"]
